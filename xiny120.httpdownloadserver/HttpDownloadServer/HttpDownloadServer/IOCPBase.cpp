...
void speedlimit::cccame(const int64_t& userid){ 
	if (userid <= 0) return; 
	{cclock lock(mmutex); mlimit[userid] += 1; } 
	cc::speedreset(); 
}

void speedlimit::ccgone(const int64_t& userid){
	if (userid <= 0) return; 
	{cclock lock(mmutex); mlimit[userid] -= 1; if (mlimit[userid] <= 0) mlimit.erase(userid); }
	cc::speedreset(); 
}

LPFN_DISCONNECTEX iocpbase::mDisConnectEx = NULL;
...
iocpbase::iocpbase() : msocketInit(false){
	...
	if ((Result = WSAStartup(MAKEWORD(2, 2), &wsaData)) != 0){ printf("WSAStartup failed with error %d\n", Result);	return;	}
			...
			if (events.iErrorCode[FD_ACCEPT_BIT] == 0 && maccept && !mshutdown){
				...
				if (INVALID_SOCKET != (clientSocket = WSAAccept(msocketListener, NULL, &nLen, 0, 0))){
					...
					if (SOCKET_ERROR != setsockopt(clientSocket, IPPROTO_TCP, TCP_NODELAY, &chOpt, sizeof(char))){		// 关闭合并小块算法。
						...
						p = inet_ntoa(sa); bSuccessed = TRUE;
					}else ::closesocket(clientSocket);
				}
			}
		}
		cclock lock(cc::ccmutex); //printf(("用户：%s[%d] Accept Status：%d\r\n"), p, cc::cconlines.size(), bSuccessed);
	}
}
...
void iocpbase::release(LPOVERLAPPED pbuf){
	...
	switch (pbase->getop()){
	case optype::init: delete (initbuf*)pbuf; break;
	case optype::read: delete (readbuf*)pbuf; break;
	case optype::write: delete (writebuf*)pbuf;	break;
	case optype::write2all:{
		write2allbuf* pall = (write2allbuf*)pbuf;
		pall->dec();
	}break;
	case optype::disconnectex: delete (disconnectexbuf*)pbuf; break;
	default:
		assert(0);
		break;
	}
...
bool iocpbase::ProcessIOMessage(LPOVERLAPPED pBuff, cc *pContext, uint32_t dwSize){
	...
	switch (pbase->getop()){
	case optype::init: bRet = oninit(pContext, dwSize, pBuff); iError = 1;  break;
